...
import com.example.mvvm_practice.data.room.LocalUserDatabase.Companion.getSampleUserList
...
class LocalUserCursorDatabase(context: Context, private val scope: CoroutineScope) : LocalUserDao,
    ...
    override fun onCreate(db: SQLiteDatabase) {
        ...
        getSampleUserList().forEach { user ->
    ...
    override fun getLocalUsersASC(): Flow<List<LocalUser>> = flow {
    ...
    override fun getLocalUsersDESC(): Flow<List<LocalUser>> = flow {
        emitAll(content.asFlow())
    }.stateIn(
        scope = scope,
        started = SharingStarted.WhileSubscribed(5000L),
        initialValue = emptyList()
    ).transformLatest { emit(it.reversed()) }
